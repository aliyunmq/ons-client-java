...
public class ProducerDelayMessageExample {
    ...
    public static void main(String[] args) {
        ...
        Message message = new Message(
            // 定时/延时消息所属的 Topic ，切勿使用定时/延时消息的 Topic 来收发其他类型的消息。
            topic,
            // Message Tag 可理解为 Gmail 中的标签，对消息进行再归类，方便 Consumer 指定过滤条件在消息队列RocketMQ版的服务器过滤。
            tag,
        ...
        // 延时消息，在指定延迟时间（当前时间之后）进行投递。单位毫秒（ms）。
        // 以下示例表示消息在 3 秒后投递
        long delayTimestampMillis = new Date().getTime() + Duration.ofSeconds(3).get(ChronoUnit.MILLIS);
        message.setStartDeliverTime(delayTimestampMillis);
        try {
            SendResult sendResult = producer.send(message);
            String messageId = sendResult.getMessageId();
            logger.info("Send DELAY message successfully, topic={}, messageId={}", topic, messageId);
        } catch (Exception e) {
            // 消息发送失败，需要进行重试处理，可重新发送这条消息或持久化这条数据进行补偿处理。
            logger.error("Failed to send DELAY message, topic={}", message.getTopic(), e);
        }
