...
public class ProducerTransactionMessageExample {
    ...
    public static void main(String[] args) {
        ...
        TransactionProducer producer = ONSFactory.createTransactionProducer(properties, message -> {
            final String topic = message.getTopic();
            final String messageId = message.getMsgID();
            logger.info("Received transaction check request, topic={}, messageId={}", topic, messageId);
        ...
        Message message = new Message(
            // 事务消息所属的 Topic，切勿使用事务消息的 Topic 来收发其他类型的消息。
            topic,
            ...
            "This is a TRANSACTION message for RocketMQ".getBytes());
        ...
        try {
            SendResult sendResult = producer.send(message, (msg, arg) -> {
                logger.info("Execute local transaction and submit status");
                return TransactionStatus.CommitTransaction;
            }, null);
            final String messageId = sendResult.getMessageId();
            logger.info("Send TRANSACTION message successfully, topic={}, messageId={}", topic, messageId);
        } catch (ONSClientException e) {
            // 消息发送失败，需要进行重试处理，可重新发送这条消息或持久化这条数据进行补偿处理。
            logger.error("Failed to send TRANSACTION message, topic={}", message.getTopic(), e);
        }
    }
}
